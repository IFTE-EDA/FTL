...
class ZBend(Transformation):
    ...
    def getScope(self):
        if (self.dir == DIR.NEGY or self.dir == DIR.POSY):
            return Rectangle((self.xmin-self.parent.mel, self.ymin-self.parent.mel), (self.xmax+self.parent.mel, self.ymin+self.parent.mel)).extrude(self.parent.zmax-self.parent.zmin+2*self.parent.mel).z(self.parent.zmin-self.parent.mel)
        elif (self.dir == DIR.NEGX or self.dir == DIR.POSX):
            return Rectangle((self.xmin-self.parent.mel, self.ymin-self.parent.mel), (self.xmin+self.parent.mel, self.ymax+self.parent.mel)).extrude(self.parent.zmax-self.parent.zmin+2*self.parent.mel).z(self.parent.zmin-self.parent.mel)
    ...
    def getBorderScope(self, delta=None):
        ...
        if (self.dir == DIR.NEGY):
            return [min(self.xmin, self.xmax) - delta, self.ymin + delta, max(self.xmin, self.xmax) + delta, self.ymin - delta]
        elif (self.dir == DIR.POSY):
            return [min(self.xmin, self.xmax) - delta, self.ymin - delta, max(self.xmin, self.xmax) + delta, self.ymin + delta]
        elif (self.dir == DIR.NEGX):
            return [self.xmin - delta, min(self.ymin, self.ymax) - delta, self.xmin + delta, max(self.ymin, self.ymax) + delta]
